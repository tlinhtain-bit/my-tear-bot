...
async def start(update, context):
    ...
    await update.message.reply_text("🎀Cherryhin Bot အသင့်ရှိပါပြီ။", reply_markup=InlineKeyboardMarkup(kb))
...
async def handle_message(update, context):
    ...
    if context_step.get(uid) == "photo" and update.message.photo:
        ...
        await update.message.reply_text("📸 ဓာတ်ပုံရပါပြီ။ Name, Age, နေရပ်, Bio ရေးပေးပါ။")
    elif context_step.get(uid) == "info" and update.message.text:
        users_db[uid]["info"] = update.message.text
        context_step[uid] = "finished"
        await update.message.reply_text("✅ မှတ်သားပြီးပါပြီ! /find နှိပ်ပါ။")
...
bot_app.add_handler(CommandHandler("find", find_love))
...
if __name__ == '__main__':
    bot_app.bot.set_webhook(f"{WEBHOOK_URL}/{TOKEN}")
