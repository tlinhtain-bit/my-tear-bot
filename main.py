...
async def track_members(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if u.chat_member.new_chat_members:
        ...
        await chat.send_message(f"✨ မင်္ဂလာပါ {user.first_name} ရေ... ကြိုဆိုပါတယ်။ 🥰")
...
# Web Server Application တည်ဆောက်ခြင်း
async def init_app():
    ...
    # Render Webhook URL ချိတ်ဆက်ခြင်း
...
# Gunicorn အတွက် entry point
def create_app():
    loop = asyncio.get_event_loop()
    return loop.run_until_complete(init_app())
